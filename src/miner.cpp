...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EurocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EurocoinMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("EurocoinMiner started\n");
    ...
    try { while (true) {
        ...
        LogPrintf("Running EurocoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("EurocoinMiner terminated\n");
        throw;
    }
}
